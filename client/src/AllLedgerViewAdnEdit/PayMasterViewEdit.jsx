...
const PayMasterViewEdit = () => {
  ...
  const [updateJournalVoucher] = useUpdatePaymasterVoucherMutation();
  const [deleteJournalVoucher] = useDeletePaymasterVoucherMutation();
  ...
  const handleAmountChange = ({ index, ledgerType, value }) => {
    ...
    const updatedLedgers = [...voucherData[ledgerType]];
    console.log("updatedLedgersvalue", updatedLedgers);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      refetchVoucherData();
      navigate("/staff/paymasterpriview"); // Redirect after update
    } catch (error) {
  ...
  const handleDelete = async () => {
    if (transactionId) {
      try {
        ...
        navigate("/staff/paymasterpriview"); // Redirect after delete
      } catch (error) {
      ...
      <main className="flex-grow flex justify-center items-center p-4">
        <div className={containerClass}>
          <h2 className="text-3xl font-bold mb-6 text-center">
            {transactionId
              ? "Edit Paymaster Voucher"
              : "Update Paymaster Voucher"}
          </h2>
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            {voucherData.debitLedgers.map((debitLedger, index) => (
              <div key={index} className="grid grid-cols-4 gap-4 items-center">
                {console.log("ledgerId", debitLedger.ledgerId)}
...
export default PayMasterViewEdit;
